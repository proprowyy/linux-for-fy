 ...
 */
struct ov5640_sensor_data {
	struct sensor_data base;
...
static struct ov5640_sensor_data ov5640_data;
...
{
	struct ov5640_sensor_data *sensor = s->priv;
...
static int ioctl_g_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
	...
	case V4L2_CID_BRIGHTNESS:
		vc->value = ov5640_data.priv.brightness;
		break;
	case V4L2_CID_HUE:
		vc->value = ov5640_data.priv.hue;
		break;
	case V4L2_CID_CONTRAST:
		vc->value = ov5640_data.priv.contrast;
		break;
	case V4L2_CID_SATURATION:
		vc->value = ov5640_data.priv.saturation;
		break;
	case V4L2_CID_RED_BALANCE:
		vc->value = ov5640_data.priv.red;
		break;
	case V4L2_CID_BLUE_BALANCE:
		vc->value = ov5640_data.priv.blue;
		break;
	case V4L2_CID_EXPOSURE:
		vc->value = ov5640_data.priv.ae_mode;
		break;
	default:
		ret = -EINVAL;
	}
...
static int ioctl_dev_init(struct v4l2_int_device *s)
{
	...
	clk_set_rate(ov5640_data.base.sensor_clk, ov5640_data.mclk);
			...
			const struct i2c_device_id *id)
{
	...
	ov5640_data.base.sensor_clk = devm_clk_get(dev, "csi_mclk");
	if (IS_ERR(ov5640_data.base.sensor_clk)) {
		dev_err(dev, "get mclk failed\n");
		return PTR_ERR(ov5640_data.base.sensor_clk);
	}
	...
	retval = of_property_read_u32(dev->of_node, "mclk_source",
					(u32 *) &(ov5640_data.base.mclk_source));
	if (retval) {
	...
	retval = of_property_read_u32(dev->of_node, "csi_id",
					&(ov5640_data.base.csi));
	if (retval) {
		dev_err(dev, "csi_id invalid\n");
		return retval;
	}

	clk_prepare_enable(ov5640_data.base.sensor_clk);

	ov5640_data.base.io_init = ov5640_reset;
	...
	if (retval < 0 || chip_id_high != 0x56) {
		clk_disable_unprepare(ov5640_data.base.sensor_clk);
