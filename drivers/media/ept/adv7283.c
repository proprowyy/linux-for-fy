...
#define ADV7280_INPUT_CONTROL_REG			0x00
#define ADV7280_INPUT_CONTROL_AD_PAL_BG_NTSC_J_SECAM	0x00
#define ADV7280_INPUT_CONTROL_AD_PAL_BG_NTSC_J_SECAM_PED 0x10
#define ADV7280_INPUT_CONTROL_AD_PAL_N_NTSC_J_SECAM	0x20
#define ADV7280_INPUT_CONTROL_AD_PAL_N_NTSC_M_SECAM	0x30
#define ADV7280_INPUT_CONTROL_NTSC_J			0x40
...
#define ADV7280_INPUT_CONTROL_PAL_SECAM_PED		0xf0
#define ADV7280_INPUT_CONTROL_INSEL_MASK		0x0f

#define ADV7280_EXTENDED_OUTPUT_CONTROL_REG		0x04
#define ADV7280_EXTENDED_OUTPUT_CONTROL_NTSCDIS		0xC5

#define ADV7280_AUTODETECT_ENABLE_REG			0x07
#define ADV7280_AUTODETECT_DEFAULT			0x7f
/* Contrast */
#define ADV7280_CON_REG		0x08	/*Unsigned */
#define ADV7280_CON_MIN		0
#define ADV7280_CON_DEF		128
#define ADV7280_CON_MAX		255
/* Brightness*/
#define ADV7280_BRI_REG		0x0a	/*Signed */
#define ADV7280_BRI_MIN		-128
#define ADV7280_BRI_DEF		0
#define ADV7280_BRI_MAX		127
/* Hue */
#define ADV7280_HUE_REG		0x0b	/*Signed, inverted */
#define ADV7280_HUE_MIN		-127
#define ADV7280_HUE_DEF		0
#define ADV7280_HUE_MAX		128

#define ADV7280_ADI_CTRL_REG				0x0e
#define ADV7280_ADI_CTRL_IRQ_SPACE			0x20

#define ADV7280_PWR_MAN_REG		0x0f
#define ADV7280_PWR_MAN_ON		0x04
#define ADV7280_PWR_MAN_OFF		0x24
#define ADV7280_PWR_MAN_RES		0x80

#define ADV7280_STATUS1_REG				0x10
...
#define ADV7280_IDENT_REG 0x11
#define ADV7280_ID_7280 0x18

#define ADV7280_ICONF1_ADI		0x40
#define ADV7280_ICONF1_ACTIVE_LOW	0x01
#define ADV7280_ICONF1_PSYNC_ONLY	0x10
#define ADV7280_ICONF1_ACTIVE_TO_CLR	0xC0
/* Saturation */
#define ADV7280_SD_SAT_CB_REG	0xe3	/*Unsigned */
#define ADV7280_SD_SAT_CR_REG	0xe4	/*Unsigned */
#define ADV7280_SAT_MIN		0
#define ADV7280_SAT_DEF		128
#define ADV7280_SAT_MAX		255

#define ADV7280_IRQ1_LOCK	0x01
#define ADV7280_IRQ1_UNLOCK	0x02
#define ADV7280_ISR1_ADI	0x42
#define ADV7280_ICR1_ADI	0x43
...
#define ADV7280_NTSC_V_BIT_END_REG	0xE6
#define ADV7280_NTSC_V_BIT_END_MANUAL_NVEND	0x4F
...
#define to_adv7280_sd(_ctrl) (&container_of(_ctrl->handler,		\
					    struct adv7280_state,	\
					    ctrl_hdl)->sd)
...
static int __adv7280_status(struct i2c_client *client, u32 *status,
			    v4l2_std_id *std)
{
	int status1 = i2c_smbus_read_byte_data(client, ADV7280_STATUS1_REG);
...
static int adv7280_s_routing(struct v4l2_subdev *sd, u32 input,
			     u32 output, u32 config)
{
	 ...
	 */
	if ((input & ADV7280_INPUT_CONTROL_INSEL_MASK) != input)
		goto out;

	ret = i2c_smbus_read_byte_data(client, ADV7280_INPUT_CONTROL_REG);

	if (ret < 0)
		goto out;

	ret &= ~ADV7280_INPUT_CONTROL_INSEL_MASK;
	ret = i2c_smbus_write_byte_data(client,
					ADV7280_INPUT_CONTROL_REG, ret | input);
	state->input = input;
out:
	mutex_unlock(&state->mutex);
	return ret;
...
static int adv7280_s_std(struct v4l2_subdev *sd, v4l2_std_id std)
{
	...
	if (std == V4L2_STD_ALL) {
		ret =
		    i2c_smbus_write_byte_data(client, ADV7280_INPUT_CONTROL_REG,
...
static int adv7280_s_ctrl(struct v4l2_ctrl *ctrl)
{
	struct v4l2_subdev *sd = to_adv7280_sd(ctrl);
	struct adv7280_state *state = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret = mutex_lock_interruptible(&state->mutex);
	int val;
	...
	case V4L2_CID_BRIGHTNESS:
		ret = i2c_smbus_write_byte_data(client, ADV7280_BRI_REG, val);
		break;
	case V4L2_CID_HUE:
		/*Hue is inverted according to HSL chart */
		ret = i2c_smbus_write_byte_data(client, ADV7280_HUE_REG, -val);
		break;
	case V4L2_CID_CONTRAST:
		ret = i2c_smbus_write_byte_data(client, ADV7280_CON_REG, val);
		break;
	case V4L2_CID_SATURATION:
		 ...
		 */
		ret = i2c_smbus_write_byte_data(client, ADV7280_SD_SAT_CB_REG,
						val);
		if (ret < 0)
			break;
		ret = i2c_smbus_write_byte_data(client, ADV7280_SD_SAT_CR_REG,
						val);
		break;
	default:
		ret = -EINVAL;
...
static const struct v4l2_ctrl_ops adv7280_ctrl_ops = {
	.s_ctrl = adv7280_s_ctrl,
};

static int adv7280_init_controls(struct adv7280_state *state)
{
	v4l2_ctrl_handler_init(&state->ctrl_hdl, 4);

	v4l2_ctrl_new_std(&state->ctrl_hdl, &adv7280_ctrl_ops,
			  V4L2_CID_BRIGHTNESS, ADV7280_BRI_MIN,
			  ADV7280_BRI_MAX, 1, ADV7280_BRI_DEF);
	v4l2_ctrl_new_std(&state->ctrl_hdl, &adv7280_ctrl_ops,
			  V4L2_CID_CONTRAST, ADV7280_CON_MIN,
			  ADV7280_CON_MAX, 1, ADV7280_CON_DEF);
	v4l2_ctrl_new_std(&state->ctrl_hdl, &adv7280_ctrl_ops,
			  V4L2_CID_SATURATION, ADV7280_SAT_MIN,
			  ADV7280_SAT_MAX, 1, ADV7280_SAT_DEF);
	v4l2_ctrl_new_std(&state->ctrl_hdl, &adv7280_ctrl_ops,
			  V4L2_CID_HUE, ADV7280_HUE_MIN,
			  ADV7280_HUE_MAX, 1, ADV7280_HUE_DEF);
...
static void adv7280_exit_controls(struct adv7280_state *state)
{
	v4l2_ctrl_handler_free(&state->ctrl_hdl);
}
...
static void adv7280_work(struct work_struct *work)
{
	...
	i2c_smbus_write_byte_data(client, ADV7280_ADI_CTRL_REG,
				  ADV7280_ADI_CTRL_IRQ_SPACE);
	isr3 = i2c_smbus_read_byte_data(client, ADV7280_ISR3_ADI);
	/* clear */
	i2c_smbus_write_byte_data(client, ADV7280_ICR3_ADI, isr3);
	i2c_smbus_write_byte_data(client, ADV7280_ADI_CTRL_REG, 0);
...
static int init_device(struct i2c_client *client, struct adv7280_state *state)
{
	int ret;
	...
	if (state->autodetect) {
		...
		ret =
		    i2c_smbus_write_byte_data(client,
					      ADV7280_AUTODETECT_ENABLE_REG,
					      ADV7280_AUTODETECT_DEFAULT);
		if (ret < 0)
			return ret;
	} else {
		ret = v4l2_std_to_adv7280(state->curr_norm);
		if (ret < 0)
			return ret;
	...
	ret = i2c_smbus_write_byte_data(client,
			ADV7280_EXTENDED_OUTPUT_CONTROL_REG,
	...
	ret = i2c_smbus_write_byte_data(client,
					ADV7280_NTSC_V_BIT_END_REG,
					ADV7280_NTSC_V_BIT_END_MANUAL_NVEND);
	if (ret < 0)
		return ret;
	...
	if (state->irq > 0) {
		ret = request_irq(state->irq, adv7280_irq, 0, KBUILD_MODNAME,
				  state);
		if (ret)
			return ret;

		ret = i2c_smbus_write_byte_data(client, ADV7280_ADI_CTRL_REG,
						ADV7280_ADI_CTRL_IRQ_SPACE);
		if (ret < 0)
			return ret;

		/* config the Interrupt pin to be active low */
		ret = i2c_smbus_write_byte_data(client, ADV7280_ICONF1_ADI,
						ADV7280_ICONF1_ACTIVE_LOW |
						ADV7280_ICONF1_PSYNC_ONLY);
		if (ret < 0)
			return ret;

		ret = i2c_smbus_write_byte_data(client, ADV7280_IMR1_ADI, 0);
		if (ret < 0)
			return ret;

		ret = i2c_smbus_write_byte_data(client, ADV7280_IMR2_ADI, 0);
		if (ret < 0)
			return ret;

		/* enable AD change interrupts interrupts */
		ret = i2c_smbus_write_byte_data(client, ADV7280_IMR3_ADI,
						ADV7280_IRQ3_AD_CHANGE);
		if (ret < 0)
			return ret;

		ret = i2c_smbus_write_byte_data(client, ADV7280_IMR4_ADI, 0);
		if (ret < 0)
			return ret;
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = adv7280_init_controls(state);
	if (ret){
		printk(KERN_ERR KBUILD_MODNAME ": Failed to probe: dv7280_init_controls(%d) \n", ret);
		goto err_unreg_subdev;
	}
	ret = init_device(client, state);
	if (ret){
		printk(KERN_ERR KBUILD_MODNAME ": Failed to probe: init_device(%d) \n", ret);
		goto err_free_ctrl;
	}
	return 0;

err_free_ctrl:
	adv7280_exit_controls(state);
err_unreg_subdev:
...
#ifdef CONFIG_PM
static int adv7280_suspend(struct i2c_client *client, pm_message_t state)
{
	int ret;

	ret = i2c_smbus_write_byte_data(client, ADV7280_PWR_MAN_REG,
					ADV7280_PWR_MAN_OFF);
	if (ret < 0)
		return ret;
	return 0;
...
static int adv7280_resume(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct adv7280_state *state = to_state(sd);
	int ret;
...
#ifdef CONFIG_PM
	.suspend = adv7280_suspend,
	.resume = adv7280_resume,
#endif
	.id_table = adv7280_id,
};

module_i2c_driver(adv7280_driver);
