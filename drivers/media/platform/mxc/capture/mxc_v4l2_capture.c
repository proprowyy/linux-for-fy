...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	if (cam->ipu == NULL) {
		pr_err("ERROR: v4l2 capture: failed to get ipu (ipu_id=%d)\n", ipu_id);
		return -EINVAL;
	} else if (cam->ipu == ERR_PTR(-ENODEV)) {
		pr_err("ERROR: v4l2 capture: get invalid ipu (ipu_id=%d)\n", ipu_id);
		return -ENODEV;
	}
...
static int mxc_v4l2_probe(struct platform_device *pdev)
{
	...
	pr_debug("enter mxc_v412_probe\n");
	if (cam == NULL) {
		pr_err("ERROR: v4l2 capture: failed to register camera\n");
		return -1;
	}

	pr_debug("mxc_v412_probe: 2\n");
	init_camera_struct(cam, pdev);
	pdev->dev.release = camera_platform_release;

	/* Set up the v4l2 device and register it*/
	cam->self->priv = cam;
	pr_debug("mxc_v412_probe: 3\n");
	v4l2_int_device_register(cam->self);

	pr_debug("mxc_v412_probe: 5\n");
	...
	pr_debug("mxc_v412_probe: 6\n");
	if (device_create_file(&cam->video_dev->dev,
			&dev_attr_fsl_v4l2_capture_property))
		dev_err(&pdev->dev, "Error on creating sysfs file"
			" for capture\n");

	pr_debug("mxc_v412_probe: 7\n");
	if (device_create_file(&cam->video_dev->dev,
			&dev_attr_fsl_v4l2_overlay_property))
		dev_err(&pdev->dev, "Error on creating sysfs file"
			" for overlay\n");

	pr_debug("mxc_v412_probe: 8\n");
	if (device_create_file(&cam->video_dev->dev,
