...
static int prp_enc_setup(cam_data *cam)
{
	...
	pr_debug("ipu_prp_enc: (%s)  CSI_PRP_ENC_MEM (%d)\n", __func__, CSI_PRP_ENC_MEM);
	...
	if (cam->rotation >= IPU_ROTATE_90_RIGHT) {
		...
		pr_debug("ipu_prp_enc: (%s)  MEM_ROT_ENC_MEM (%d)\n", __func__, MEM_ROT_ENC_MEM);
...
static int prp_enc_enabling_tasks(void *private)
{
	...
	if (cam->rotation >= IPU_ROTATE_90_RIGHT) {
		pr_info("%s (IIPU_IRQ_PRP_ENC_ROT_OUT_EOF) ", __func__);
		err = ipu_request_irq(cam->ipu, IPU_IRQ_PRP_ENC_ROT_OUT_EOF,
				      prp_enc_callback, 0, "Mxc Camera", cam);
	} else {
		pr_info("%s (IPU_IRQ_PRP_ENC_OUT_EOF) ", __func__);
