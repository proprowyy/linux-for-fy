...
static void vadc_get_std(struct vadc_data *vadc, v4l2_std_id *std)
{
	...
	if (*std != vadc->std_id) {
		video_idx = idx;
		vadc->std_id = *std;
		vadc->priv.pix.width = video_fmts[video_idx].active_width;
		vadc->priv.pix.height = video_fmts[video_idx].active_height;
	}
...
static int ioctl_g_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		pr_debug("   Returning size of %dx%d\n",
			 vadc->priv.pix.width, vadc->priv.pix.height);
		f->fmt.pix = vadc->priv.pix;
		break;
...
static int ioctl_g_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
	...
	case V4L2_CID_BRIGHTNESS:
		vc->value = vadc->priv.brightness;
		break;
	case V4L2_CID_HUE:
		vc->value = vadc->priv.hue;
		break;
	case V4L2_CID_CONTRAST:
		vc->value = vadc->priv.contrast;
		break;
	case V4L2_CID_SATURATION:
		vc->value = vadc->priv.saturation;
		break;
	case V4L2_CID_RED_BALANCE:
		vc->value = vadc->priv.red;
		break;
	case V4L2_CID_BLUE_BALANCE:
		vc->value = vadc->priv.blue;
		break;
	case V4L2_CID_EXPOSURE:
		vc->value = vadc->priv.ae_mode;
		break;
	default:
		ret = -EINVAL;
	}
			      ...
			      struct v4l2_fmtdesc *fmt)
{
	...
	fmt->pixelformat = vadc->priv.pix.pixelformat;
	return 0;
}
...
static int vadc_probe(struct platform_device *pdev)
{
	...
	vadc->priv.streamcap.timeperframe.denominator = 30;
	vadc->priv.streamcap.timeperframe.numerator = 1;
	vadc->std_id = V4L2_STD_ALL;
	video_idx = VADC_NTSC;
	vadc->priv.pix.width = video_fmts[video_idx].active_width;
	vadc->priv.pix.height = video_fmts[video_idx].active_height;
	vadc->priv.pix.pixelformat = V4L2_PIX_FMT_YUV444;  /* YUV444 */
	vadc->priv.pix.priv = 1;  /* 1 is used to indicate TV in */
	vadc->priv.on = true;
