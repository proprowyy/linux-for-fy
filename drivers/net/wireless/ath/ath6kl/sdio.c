			  ...
			  u8 *buf, u32 len)
{
	...
	if (request & HIF_WRITE) {
		...
		if (request & HIF_FIXED_ADDRESS){
			...
			//pr_info("%s:sdio_writesb(ret=%d)\n", __func__, ret);
		}else{
			ret = sdio_memcpy_toio(func, addr, buf, len);
			//pr_info("%s:sdio_memcpy_toio(ret=%d)\n", __func__, ret);
		}
	} else {
		if (request & HIF_FIXED_ADDRESS){
			ret = sdio_readsb(func, buf, addr, len);
			//pr_info("%s:sdio_readsb(ret=%d)\n", __func__, ret);
		}else{
			ret = sdio_memcpy_fromio(func, buf, addr, len);
			//pr_info("%s:sdio_memcpy_fromio(ret=%d)\n", __func__, ret);
		}
	}
...
{
	int ret;
	ath6kl_dbg(ATH6KL_DBG_BOOT,"enter ath6kl_sdio_init\n");
	pr_debug("enter ath6kl_sdio_init\n");

	ret = sdio_register_driver(&ath6kl_sdio_driver);
	if (ret)
		ath6kl_err("sdio driver registration failed: %d\n", ret);

	ath6kl_dbg(ATH6KL_DBG_BOOT,"leave ath6kl_sdio_init\n");
	pr_debug("leave ath6kl_sdio_init\n");
