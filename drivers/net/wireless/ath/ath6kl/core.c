...
int ath6kl_core_init(struct ath6kl *ar, enum ath6kl_htc_type htc_type)
{
	...
	pr_info("%s: ath6kl_hif_power_on(%d)\n", __func__, ret);
	if (ret)
		goto err_bmi_cleanup;

	ret = ath6kl_bmi_get_target_info(ar, &targ_info);
	pr_info("%s: ath6kl_bmi_get_target_info(%d)\n", __func__, ret);
	if (ret)
		goto err_power_off;
	...
	pr_info("%s: ath6kl_init_hw_params(%d)\n", __func__, ret);
	if (ret)
		goto err_power_off;
	...
	pr_info("%s: ath6kl_init_fetch_firmwares(%d)\n", __func__, ret);
	if (ret)
	...
	pr_info("%s: ath6kl_init_hw_start(%d)\n", __func__, ret);
	if (ret) {
	...
	pr_info("%s: ath6kl_cfg80211_init(%d)\n", __func__, ret);
	if (ret)
		goto err_rxbuf_cleanup;

	ret = ath6kl_debug_init_fs(ar);
	pr_info("%s: ath6kl_debug_init_fs(%d)\n", __func__, ret);
	if (ret) {
