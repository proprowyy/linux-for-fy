...
struct knob_device{
	spinlock_t spinlock;
	int pin, cwpin, acwpin;
	int irq, cwpin_irq, acwpin_irq;
...
{  
    ...
    //input_report_key(knob->input_dev, KNOB_KEY, 1);
...
static irqreturn_t knob_clockwise_irq(int irq, void *dev_id)
{
	...
	data->knob_clockwise_status = true;

	if((true == data->knob_clockwise_status ) && (true == data->knob_anticlockwise_status))
	{
...
static irqreturn_t knob_anticw_irq(int irq, void *dev_id)
{
	...
	data->knob_anticlockwise_status = true;
...
static int knob_init_irq(struct knob_device* knob, struct device *dev)
{
	...
	err = request_irq(knob->cwpin_irq, knob_clockwise_irq, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "knob_clockwise_irq", knob);
	if (err) {
		pr_info("failed to request IRQ1 \n");
		return -1;
	}
	
	err = request_irq(knob->acwpin_irq, knob_anticw_irq, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "knob_anticw_irq", knob);
	if (err) {
