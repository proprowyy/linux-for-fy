...
struct isl29023_data *pisl29023_data;
/*
...
static struct i2c_client* to_this_i2c_client(struct device *dev)
{
	//return to_i2c_client(dev->parent);
	return pisl29023_data->client;
}
...
static ssize_t isl29023_show_int_persists(struct device *dev,
					  ...
					  char *buf)
{
	struct i2c_client *client = to_this_i2c_client(dev);
...
#if 1 // unmarked by yusw			
        /* register sysfs hooks */
        err = sysfs_create_group(&input_dev->dev.kobj, &isl29023_attr_group);
        if (err)
                goto exit_kfree;
