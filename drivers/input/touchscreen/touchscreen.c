	...
	unsigned int *value)
{
	...
	error = of_property_read_u32(of_node, property, &val);
	if (error) {
		*value = default_value;
		return false;
	}
...
struct touchscreen_properties *prop)
{
	...
	prop->invert_x =
		of_property_read_bool(of_node, "touchscreen-inverted-x");
	prop->invert_y =
		of_property_read_bool(of_node, "touchscreen-inverted-y");
	prop->swap_x_y =
		of_property_read_bool(of_node, "touchscreen-swapped-x-y");
