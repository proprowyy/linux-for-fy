...
static int imx_pwm_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "pwm_probe 1\n");
	if (!of_id)
		return -ENODEV;

	dev_dbg(&pdev->dev, "pwm_probe 2\n");
	...
	dev_dbg(&pdev->dev, "pwm_probe 6\n");
	ret = pwmchip_add(&imx->chip);
	if (ret < 0)
		return ret;

	dev_dbg(&pdev->dev, "pwm_probe 7\n");
