...
static void mxc_hdmi_setup(struct mxc_hdmi *hdmi, unsigned long event)
{
	...
	dev_dbg(&hdmi->pdev->dev, "enter %s (%u)\n", __func__, (unsigned int)event);
					...
					unsigned long val, void *v)
{
	...
	case FB_EVENT_BLANK:
		dev_dbg(&hdmi->pdev->dev, "event=FB_EVENT_BLANK\n");
	...
	case FB_EVENT_SUSPEND:
		dev_dbg(&hdmi->pdev->dev,"event=FB_EVENT_SUSPEND\n");
	...
	case FB_EVENT_RESUME:
		dev_dbg(&hdmi->pdev->dev,"event=FB_EVENT_RESUME\n");
			      ...
			      struct mxc_dispdrv_setting *setting)
{
	...
	dev_dbg(&hdmi->pdev->dev, "init DDC pins for HDCP (hdcp_init=%d)\n", hdcp_init);
	if (hdcp_init) {
	...
	dev_dbg(&hdmi->pdev->dev, "fb_find_mode\n");
	...
	/*Add all no interlaced CEA mode to default modelist */
	dev_dbg(&hdmi->pdev->dev, "fb_add_videomode\n");
	for (i = 0; i < ARRAY_SIZE(mxc_cea_mode); i++) {
		mode = &mxc_cea_mode[i];
		if (!(mode->vmode & FB_VMODE_INTERLACED) && (mode->xres != 0))
			fb_add_videomode(mode, &hdmi->fbi->modelist);
	...
	dev_dbg(&hdmi->pdev->dev, "fb_var_to_videomode\n");
	fb_var_to_videomode(&m, &hdmi->fbi->var);
	dump_fb_videomode(&m);
	...
	dev_dbg(&hdmi->pdev->dev, "fb_find_nearest_mode\n");
	mode = fb_find_nearest_mode(&m, &hdmi->fbi->modelist);
	if (!mode) {
		pr_err("%s: could not find mode in modelist\n", __func__);
		return -1;
	...
	dev_dbg(&hdmi->pdev->dev, "fb_videomode_to_var\n");
	fb_videomode_to_var(&hdmi->fbi->var, mode);
