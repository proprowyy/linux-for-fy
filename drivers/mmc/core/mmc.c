...
int mmc_set_ext_csd(struct mmc_card *card, u8 *ext_csd);
...
static int mmc_compare_ext_csds(struct mmc_card *card, unsigned bus_width)
{
	...
	pr_info("==========enter mmc_compare_ext_csds \r\n");
			...
			struct device_attribute *attr, char *buf)
{
	...
	pr_info("==========enter mmc_boot_info_show \r\n");

	/* read it again because user may change it */
	mmc_claim_host(card->host);
	err = mmc_get_ext_csd(card, &ext_csd);
// YUSW
//	pr_info("==========enter mmc_send_ext_csd \r\n");

//	ext_csd[EXT_CSD_REV] = 6; // EXT_CSD_REV = 192
		...
		const char *buf, size_t count)
{
	...
	BUG_ON(!card);
	pr_info("==========enter setup_boot_partitions \r\n");
	...
	pr_info("==========enter mmc_send_ext_csd 2 \r\n");
	err = mmc_send_ext_csd(card, ext_csd);
	if (err) {
		...
		const char *buf, size_t count)
{
	...
	BUG_ON(!card);
	pr_info("==========enter setup_boot_bus 2 \r\n");
	...
	struct mmc_card *oldcard)
{
	...
	if (!oldcard) {
		...
		/*
		 * Make Sumsang EMMC work at Revision 6 (YUSW)
		 */
		ext_csd[EXT_CSD_REV] = 6; // EXT_CSD_REV = 192
		mmc_set_ext_csd(card, ext_csd);

		if (err)
			goto free_card;
		err = mmc_read_ext_csd(card, ext_csd);
	pr_info("==========enter mmc_send_ext_csd 222 \r\n");
		if (err)
			goto free_card;
