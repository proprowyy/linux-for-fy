...
static int sdhci_esdhc_imx_probe(struct platform_device *pdev)
{
	...
	pr_info("enter sdhci_esdhc_imx_probe\n");
	...
	pr_info("leave sdhci_esdhc_imx_probe\n");
	return 0;
...
free_sdhci:
	...
	pr_info("leave sdhci_esdhc_imx_probe(error)\n");
	return err;
}
...
static int queue_reference = 0;

static int sdhci_probe_work_init(void)
{
	if(queue_reference==0){
		queue_reference++;
		//queue=create_singlethread_workqueue("hello world");
		queue=create_workqueue("sdhci work");
		if(!queue)
			goto err;
	}else{
		queue_reference++;
...
static void sdhci_probe_work_exit(void)
{
	queue_reference--;
	if(queue_reference==0){
...
static struct sdhci_work work[10];  

static void work_handler(struct work_struct *work)
{
	struct sdhci_work* owner  = container_of(work, struct sdhci_work, work);
	pr_debug("sdhci work handler function.\n");
	sdhci_esdhc_imx_probe(owner->pdev);
}

static int my_sdhci_esdhc_imx_probe(struct platform_device *pdev)
{
	pr_debug("enter my_sdhci_esdhc_imx_probe\n");
	sdhci_probe_work_init();
	work[work_num].pdev = pdev;
	INIT_WORK(&work[work_num].work,work_handler);
	schedule_work(&work[work_num].work);
	work_num++;
	pr_debug("leave my_sdhci_esdhc_imx_probe\n");
	return 0;

}
static int my_sdhci_esdhc_imx_remove(struct platform_device *pdev)
{
	sdhci_esdhc_imx_remove(pdev);
	sdhci_probe_work_exit();
	return 0;
}
