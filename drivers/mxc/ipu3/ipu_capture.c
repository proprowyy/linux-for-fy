	...
	uint32_t pixel_fmt, ipu_csi_signal_cfg_t cfg_param)
{
	...
	dev_dbg(ipu->dev, "%s (width=%d, height=%d, pixel_fmt=%d) \n", __func__, width, height, pixel_fmt);
...
void ipu_csi_get_window_size(struct ipu_soc *ipu, uint32_t *width, uint32_t *height, uint32_t csi)
{
	...
	*height = (reg >> 16 & 0xFFFF) + 1;
	dev_dbg(ipu->dev, "%s (csi=%d, width=%d, height=%d) \n", __func__, csi, *width, *height);
...
void ipu_csi_set_window_pos(struct ipu_soc *ipu, uint32_t left, uint32_t top, uint32_t csi)
{
	...
	dev_dbg(ipu->dev, "%s (csi=%d, left=%d, top=%d) \n", __func__, csi, left, top);
	mutex_unlock(&ipu->mutex_lock);
	...
	uint32_t g_value, uint32_t b_value, uint32_t pix_clk, uint32_t csi)
{
	...
	dev_dbg(ipu->dev, "%s (csi=%d, rgb=[%d, %d, %d], pix_clk=%d) \n", __func__, csi, r_value, g_value, b_value, pix_clk);
...
static void _ipu_csi_test_enable(struct ipu_soc *ipu, uint32_t r_value,
	uint32_t g_value, uint32_t b_value, uint32_t csi)
{
	uint32_t temp;
	temp = ipu_csi_read(ipu, csi, CSI_TST_CTRL);
	temp &= ~(CSI_TEST_GEN_R_MASK | CSI_TEST_GEN_G_MASK |
	...
	temp |= (r_value << CSI_TEST_GEN_R_SHIFT) |
		...
		(b_value << CSI_TEST_GEN_B_SHIFT);
	ipu_csi_write(ipu, csi, temp, CSI_TST_CTRL);
	dev_dbg(ipu->dev, "%s (csi=%d, rgb=[%d, %d, %d]) \n", __func__, csi, r_value, g_value, b_value);
}

static void _ipu_csi_test_disable(struct ipu_soc *ipu, uint32_t csi)
{
	uint32_t temp;
	temp = ipu_csi_read(ipu, csi, CSI_TST_CTRL);
	temp &= ~CSI_TEST_GEN_MODE_EN;
...
int _ipu_csi_init(struct ipu_soc *ipu, ipu_channel_t channel, uint32_t csi)
{
	...
	//_ipu_csi_test_enable(ipu, 255, 0, 0, csi);
