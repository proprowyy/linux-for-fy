...
#if defined(CONFIG_MX6_CLK_FOR_BOOTUI_TRANS_HDMI_IPU2_DI0) || \
...
struct ipu_soc *ipu_get_soc(int id)
{
	if (id >= MXC_IPU_MAX_NUM){
		pr_info("ipu_get_soc: id >= MXC_IPU_MAX_NUM\n");
		return ERR_PTR(-ENODEV);
	}else if (!ipu_array[id].online){
		pr_info("ipu_get_soc: !ipu_array[id].online\n");
		return ERR_PTR(-ENODEV);
	}else{
		//pr_info("ipu_get_soc ok\n");
		return &(ipu_array[id]);
	}
}
...
static int ipu_probe(struct platform_device *pdev)
{
	...
	dev_dbg(ipu->dev, "IPU Set Platform Data");
...
#ifdef CONFIG_MX6_CLK_FOR_BOOTUI_TRANS
	/* For logo keep patch, IPU clock was enabled
	 * once in clk-imx6q.c, disable it once to
	 * make the enable count correct.
	 */
	if (pltfm_data->bypass_reset)
		clk_disable(ipu->ipu_clk);
#endif

	dev_dbg(ipu->dev, "IPU Register");
	register_ipu_device(ipu, ipu->pdata->id);
	dev_dbg(ipu->dev, "IPU Register OK");

	pm_runtime_enable(&pdev->dev);
	
	dev_dbg(ipu->dev, "IPU Probe OK");
	return ret;
}
...
int32_t ipu_init_channel(struct ipu_soc *ipu, ipu_channel_t channel, ipu_channel_params_t *params)
{
	...
	case CSI_PRP_ENC_MEM:
		dev_dbg(ipu->dev, "is CSI_PRP_ENC_MEM\n");
		...
		break;
	case CSI_PRP_VF_MEM:
		dev_dbg(ipu->dev, "is CSI_PRP_VF_MEM\n");
				...
				uint32_t u, uint32_t v)
{
	...
	uint32_t dma_chan;
	uint32_t burst_size;
	dev_dbg(ipu->dev, "ipu_init_channel_buffer\n");
...
int32_t ipu_enable_channel(struct ipu_soc *ipu, ipu_channel_t channel)
{
	...
	uint32_t sec_dma;
	uint32_t thrd_dma;
	dev_dbg(ipu->dev, "%s: channel_id(%d)\n", __func__, IPU_CHAN_ID(channel));
		...
		uint32_t bufNum)
{
	...
	uint32_t reg;
	unsigned long lock_flags;
	dev_dbg(ipu->dev, "%s: channel_id(%d), dma_chan(%d)\n", __func__, IPU_CHAN_ID(channel), dma_chan);
		...
		uint32_t bufNum)
{
	...
	dev_dbg(ipu->dev, "%s: channel_id(%d), dma_chan(%d)\n", __func__, IPU_CHAN_ID(channel), dma_ch);
		    ...
		    uint32_t irq_flags, const char *devname, void *dev_id)
{
	...
	dev_dbg(ipu->dev, "%s (%s) : irq=%d irq_flags=%d\n", __func__, devname, irq, irq_flags);
	_ipu_get(ipu);
...
int32_t ipu_init_channel_late_init(struct ipu_soc *ipu, ipu_channel_t channel, ipu_channel_params_t *params)
{
	...
	dev_dbg(ipu->dev, "init channel (late_init) = %d\n", IPU_CHAN_ID(channel));
